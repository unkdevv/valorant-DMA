...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	...
	reinit:
		...
		this->vHandle = VMMDLL_Initialize(argc, (LPSTR*)args);
		if (!this->vHandle)
		{
			if (memMap)
			{
...
bool Memory::FixCr3()
{
	...
	VfsFileList.pfnAddFile = 0; //dumb af callback who made this system
...
{
	return VMMDLL_Scatter_Initialize(this->vHandle, current_process.PID, VMMDLL_FLAG_NOCACHE);
}

VMMDLL_SCATTER_HANDLE Memory::CreateScatterHandle(int pid) const
{
	return VMMDLL_Scatter_Initialize(this->vHandle, pid, VMMDLL_FLAG_NOCACHE);
}
